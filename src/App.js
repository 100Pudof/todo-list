...
import { List, Tasks, AddButtonList } from './components/index';
...
// видимо баг Route, без location не функционирует useHistory, хотя location Нигде не задействован.
  ...
  useEffect(() => {
    ...
    if (list) {
      const items = list.find(lis => lis.id === Number(listId));
      setActiveItem(items);
    }
